...
def test_qa_evidence_check():
    ...
    from agents.engine import qc_node
    
    # 1. 构造一个缺失 evidence_ids 的 claim（触发打回失败）
    bad_extracted = {
        "provider_name": "OpenAI",
        "region": "international",
        "model_family": "gpt-5.5-instant",
        "pricing": {"prompt_price_per_million": 1.1, "completion_price_per_million": 4.4, "currency": "USD"},
        "rate_limits": {"rpm": 10000, "tpm": 1000000},
        "features": {"context_window": 200000, "function_calling": True, "vision_support": True},
        "user_feedback": {"developer_satisfaction": 4.8, "strengths": ["高智能"], "pain_points": ["价格贵"]},
        "coding_plan": {"is_supported_in_editor": True, "language_optimizations": ["Python"], "has_sandbox_env": True, "plan_description": "专属沙盒"},
        "claims": [
            {
                "claim_id": "claim_missing_ev",
                "claim_text": "本模型支持免费测试沙盒",
                "dimension": "coding_plan",
                "competitor": "OpenAI",
                "confidence": 0.95,
                "evidence_ids": []  # 故意为空，触发打回
            }
        ],
        "evidences": []
    }
    ...
    state_bad = {
        "extracted_json": bad_extracted,
        "current_competitor": "OpenAI",
        "retry_count": 0,
        "sanitized_data": "OpenAI Model is gpt-5.5-instant. Input is $1.10/1M, Output is $4.40/1M.", # 用于事实核查匹配
        "trace_logs": []
    }
    
    output_bad = qc_node(state_bad)
    assert output_bad["validation_verdict"] == False
    assert "claim_missing_ev" in output_bad["feedback"]
    assert "缺失证据链" in output_bad["feedback"]
    assert output_bad["retry_count"] == 1
    
    # 2. 构造一个置信度低且带有证据的 claim（正常放行，但产生 warning 警告）
    warn_extracted = {
        "provider_name": "OpenAI",
        "region": "international",
        "model_family": "gpt-5.5-instant",
        ...
        "claims": [
            {
                "claim_id": "claim_low_confidence",
                "claim_text": "本模型支持免费测试沙盒",
                "dimension": "coding_plan",
                "competitor": "OpenAI",
                "confidence": 0.5,  # 低置信度 (< 0.6)
                "evidence_ids": ["ev_001"]
            }
        ],
        "evidences": [
    ...
    state_warn = {
        "extracted_json": warn_extracted,
        "current_competitor": "OpenAI",
        "retry_count": 0,
        "sanitized_data": "OpenAI Model is gpt-5.5-instant. Input is $1.10/1M, Output is $4.40/1M.",
        "trace_logs": []
    }
    
    output_warn = qc_node(state_warn)
    assert output_warn["validation_verdict"] == True
    # 验证是否成功发出了 warning 并记录到 trace_logs
    trace_text = "\n".join(output_warn["trace_logs"])
    assert "[QC WARNING]" in trace_text
    assert "claim_low_confidence" in trace_text
    assert "0.5" in trace_text
    
    print("[OK] [QA 证据与置信度深度质检测试] 缺失证据触发打回、低置信度警告放行逻辑完美通过！")

if __name__ == "__main__":
    print("开始运行 HarnessFlow TDD 单元测试用例...")
    test_sanitizer_confidential_masking()
    test_qa_evidence_check()
