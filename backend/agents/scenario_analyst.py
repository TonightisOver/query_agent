...
要求：
1. 开头直接回答用户的问题，给出明确的推荐结论
2. 推荐具体厂商和模型，结合评分数据说明为什么适合该场景
3. 给出具体的使用策略（如分层路由：日常任务用A，复杂任务用B）
4. 提供成本估算参考（如"日均1万次调用约花费X元"）
5. 提醒该场景下 1-2 个需要注意的技术风险点
6. 语言专业但易懂，用数据说话，避免空洞营销话术

【极其重要原则】：
你当前的评估场景是：「{scenario_name}」（场景代码: {scenario}）。
你的回答必须高度、严密契合该特定场景的核心要求。绝对不能输出任何与当前场景无关的开发者 IDE 嵌入、CodingPlan、IDE 插件集成、函数调用工具 API 等代码开发或编程概念，除非用户当前的需求中明确要求了这些特性。"""

        if scenario == "data_analysis":
            system_prompt += """
同时，针对当前的数据分析场景，你的选型建议输出格式必须严格符合以下 Markdown 结构：
### 问题
说明该场景要解决的是 AI 数据分析助手的大模型与产品能力选型问题。

### 输出结果
给出适合该场景的核心能力维度（模型能力包括：表格理解能力、长表格处理能力、数据清洗能力、统计分析能力、趋势分析能力、异常解释能力、SQL 生成能力、Python 代码生成与执行能力、图表生成能力、自然语言报告生成能力、结构化输出能力、幻觉控制能力；产品能力包括：Excel/CSV 上传解析、数据库连接、数据预览、字段识别、自动清洗、图表配置、报表模板、分析任务历史、导出 PDF/Word/PPT、权限管理、数据脱敏、企业私有化部署、可解释性和结果可追溯）、候选模型类型、产品功能模块和推荐组合。

### 分析与整改
说明原回答为什么跑偏（过度偏向通用指标与被低相关度 RAG 噪音如 Mistral/Codestral 编程新闻等污染），以及如何避免低相关度 RAG 内容（低于 0.2）污染当前任务（引入 >= 0.2 强过滤和 target_scenario 场景隔离，微调 data_analysis 专属权重，自适应定制 Appendix C BI 报表对比大盘）。"""
