...
from schemas import CompetitorIntelligence, PricingSchema, RateLimitsSchema, FeaturesSchema, UserFeedbackSchema, CodingPlanSchema, SourceTrace, ModelIntelligenceSchema, AnalysisClaim, EvidenceItem
        ...
        for attempt in range(max_retries):
            ...
            try:
                response = llm_client.chat.completions.create(
                    ...
                    temperature=float(os.getenv("LLM_TEMPERATURE")) if os.getenv("LLM_TEMPERATURE") and os.getenv("LLM_TEMPERATURE").strip() else 0.1,
                    max_tokens=int(os.getenv("LLM_MAX_TOKENS")) if os.getenv("LLM_MAX_TOKENS") and os.getenv("LLM_MAX_TOKENS").strip() else 8192
                )
...
### 分析与整改
1. **原回答跑偏原因**：原系统在处理「AI数据分析与自动化报表」场景时，由于 RAG 检索没有对低相关度文本进行过滤，导致外部编程新闻、代码模型发布等噪音数据污染了检索上下文。同时，系统之前未能对 `data_analysis` 场景定制专属的权重指标，错误地套用了通用模型价格与多模态大盘。
2. **如何避免低相关度 RAG 污染**：
   ...
   - **动态报表定制**：在 `engine.py` 写入节点中，针对 `data_analysis` 场景自适应输出定制化的 Appendix C BI 报表厂商能力对比大盘，避免无关文档等残留内容污染。"""
            else:
    ...
    # 对获取到的竞品 fallback 数据进行深拷贝以防污染全局知识库
    import copy
    data = copy.deepcopy(data)
    
    # 默认给所有竞品注入带有证据的 claims 与 evidences 数据以实现证据驱动竞品分析
    price_val = data.get("pricing", {}).get("prompt_price_per_million", 1.0)
    currency_val = data.get("pricing", {}).get("currency", "USD")
    ...
    path_name = provider_path_map.get(current_key, current_key.lower())
    
    data["claims"] = [
        {
            "claim_id": f"claim_{path_name}_default",
            "claim_text": f"{current_key} 旗舰模型百万 Prompt Token 计费为 {price_val} {currency_val}。",
            "dimension": "pricing",
            "competitor": current_key,
            "confidence": 0.95,
            "evidence_ids": [f"ev_{path_name}_default"]
        }
    ]
    data["evidences"] = [
        {
            "evidence_id": f"ev_{path_name}_default",
            "source_type": "web",
            "title": f"{current_key} API Reference",
            "url": f"http://mock-server:8080/mock/{path_name}",
            "snippet": f"Input pricing is {price_val} per million tokens.",
            "competitor": current_key,
            "reliability_score": 0.98
        }
    ]

    # 完美保持 OpenAI 打回机制与证据链质检的完美闭环演示：
    if "OpenAI" in current_key:
        if not ("发现以下字段存在问题" in prompt or "Feedback" in prompt or "事实不匹配问题" in prompt or "QC Evidence-Missing" in prompt or "缺失证据链" in prompt):
            # Step 1: 首次提取，故意遗漏 completion_price_per_million (触发 Pydantic Schema ValidationError 打回)
            if "pricing" in data and "completion_price_per_million" in data["pricing"]:
                del data["pricing"]["completion_price_per_million"]
            return json.dumps(data, ensure_ascii=False)
            
        elif "pricing -> completion_price_per_million" in prompt and not ("QC Evidence-Missing" in prompt or "缺失证据链" in prompt):
            # Step 2: 第二次提取，补齐 completion_price_per_million 价格，但是故意少给 claim 的 evidence_ids (触发 Evidence-Missing ValueError 打回)
            # 同时注入一条低置信度 (0.5 < 0.6) 的 claim 以演示 warning 警告逻辑
            data["model_family"] = "gpt-5.5-instant"
            data["pricing"]["prompt_price_per_million"] = 1.1
            data["pricing"]["completion_price_per_million"] = 4.4
            data["claims"] = [
                {
                    "claim_id": "claim_openai_01",
                    "claim_text": "OpenAI 旗舰模型百万 Prompt Token 计费为 1.1 USD。",
                    "dimension": "pricing",
                    "competitor": "OpenAI",
                    "confidence": 0.95,
                    "evidence_ids": []  # 故意为空，触发 QA failed
                },
                {
                    "claim_id": "claim_openai_02",
                    "claim_text": "OpenAI 开发者生态整体满意度较高。",
                    "dimension": "user_feedback",
                    "competitor": "OpenAI",
                    "confidence": 0.5,   # 置信度低，触发警告而不打回
                    "evidence_ids": ["ev_openai_02"]
                }
            ]
            data["evidences"] = [
                {
                    "evidence_id": "ev_openai_02",
                    "source_type": "web",
                    "title": "OpenAI Developer Satisfaction",
                    "url": "http://mock-server:8080/mock/openai",
                    "snippet": "Satisfaction score is high.",
                    "competitor": "OpenAI",
            ...
            # Step 3: 第三次及之后提取，全部补齐，顺利 QA passed，但保留 warning 警告
            data["model_family"] = "gpt-5.5-instant"  # 纠正为语料中真实的型号，排除臆造幻觉型号
            data["pricing"]["prompt_price_per_million"] = 1.1
            data["pricing"]["completion_price_per_million"] = 4.4
            data["claims"] = [
                {
                    ...
                    "competitor": "OpenAI",
                    "confidence": 0.95,
                    "evidence_ids": ["ev_openai_01"]  # 补齐证据 ID
                },
                {
                    "claim_id": "claim_openai_02",
                    "claim_text": "OpenAI 开发者生态整体满意度较高。",
            ...
            data["evidences"] = [
                {
                    "evidence_id": "ev_openai_01",
                    "source_type": "web",
                    "title": "OpenAI Pricing Policy",
                    "url": "http://mock-server:8080/mock/openai",
                    "snippet": "Input is $1.10/1M, Output is $4.40/1M",
                    "competitor": "OpenAI",
...
def qc_node(state: AgentState) -> Dict[str, Any]:
    ...
    try:
        ...
        # 1. 提取并实例化 claims 和 evidences 列表
        extracted_claims = extracted.get("claims", [])
        claims_list = []
        for c in extracted_claims:
            claims_list.append(AnalysisClaim(
                ...
                risk_note=c.get("risk_note", None)
            ))
            
        extracted_evidences = extracted.get("evidences", [])
        evidences_list = []
        for ev in extracted_evidences:
            evidences_list.append(EvidenceItem(
                evidence_id=ev.get("evidence_id", ""),
                source_type=ev.get("source_type", ""),
                title=ev.get("title", ""),
                url=ev.get("url", ""),
                snippet=ev.get("snippet", ""),
                collected_at=ev.get("collected_at") or datetime.utcnow(),
                competitor=ev.get("competitor", ""),
                reliability_score=ev.get("reliability_score", 1.0)
            ))
        ...
        # 1.4. 启动深度证据链与置信度门禁审查 (Evidence-driven QA Auditor)
        for claim in claims_list:
            # 规则一：若无 evidence_ids 或其为空数组，QA 失败打回
            if not getattr(claim, "evidence_ids", None) or len(claim.evidence_ids) == 0:
                raise ValueError(
                    f"[QC Evidence-Missing] 分析结论 '{claim.claim_id}' 缺失证据链 (evidence_ids 不能为空，必须绑定至少一个具体的证据 ID)"
                )
            
            # 规则二：若 confidence < 0.6，给出 warning 并记录 trace_logs
            if claim.confidence < 0.6:
                warn_msg = f"[QC WARNING] 结论 '{claim.claim_id}' 的置信度 ({claim.confidence}) 低于阈值 0.6。"
                print(warn_msg)
                trace_msg += f" {warn_msg}"
...
"""
    ...
    # 新章节 3: 动态生成附录 C。仅在存在高置信度场景匹配的证据时按需生成，否则省略 (Omit) 以防空泛噪点填充
    parsed_req = state.get("parsed_requirement") or {}
    scenario = parsed_req.get("scenario", "general")
    
    has_matched = state.get("has_matched_evidence", False)
    
    # 针对数据分析场景，若无场景匹配证据，则完全省略 (omit) 附录 C 渲染
    if scenario == "data_analysis" and not has_matched:
        pass
    elif scenario == "code_development":
        report += """
---
