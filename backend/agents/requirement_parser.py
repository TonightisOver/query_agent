...
【极其重要解析原则】：
1. Current Query Priority Rule (当前用户输入最高优先级原则)：用户的“当前过滤诉求（raw_query）”是绝对最高优先级的提取来源，提供的“已知背景知识/参考快讯/历史大纲”仅作通识参考。
2. 无论背景知识中提到了什么场景（如之前的小说生成大纲、其他代码任务预算上限等），你必须 100% 仅根据当前 query 的文本句意来决定 scenario 场景分类与字段提取！
3. 严禁将背景知识中关于特定场景的限制或历史意图（如创意写作、编程等限制）直接带入或套用到当前全新的需求解析 JSON 字段中！
4. 如果用户在当前的 raw_query 中没有明确提出价格预算上限，则 budget_range 中的 max_price_per_million 必须解析为 null。
5. 如果用户在当前的 raw_query 中没有明确提到必须支持函数调用或视觉多模态，则 feature_requirements 中的对应字段 must be parsed as null.
6. 场景(scenario)智能归类规则：
   - 小说、故事、章节续写、创意写作、内容创作、文案、视频脚本 -> creative (创意写作)
   - 自动清洗、上传 Excel/CSV 或数据库数据、自动分析趋势、生成图表、解释异常、输出报告、数据分析助手、自动化报表 -> data_analysis (数据分析)
   - 论文、学术写作、文献总结、长文档处理、合同审查、简历筛选 -> document_analysis (文档分析)
   - 企业合规、安全合规、私有部署、本地化部署 -> enterprise (企业合规)
   - 代码补全、编程开发、Agent 调试、自动化编程 -> code_development (代码开发)
   - 智能客服、在线聊天、对话系统 -> customer_service (客服系统)
   - 成本优化、多模型协作、协同调度 -> general (通用场景)
...
请严格只返回该 JSON，不能有任何 Markdown 标记或多余文字。"""
    ...
    try:
        ...
        # 意图校正防偏护栏：严格贯彻“当前用户输入最高优先级”原则，若 query 中强包含特定场景特征，强行将 scenario 进行校正，防止大模型受 RAG 历史遗留背景（如小说生成大纲等）的严重漂移干扰
        query_lower = query.lower()
        if any(k in query_lower for k in ["数据分析", "excel", "csv", "sql", "pandas", "dataframe", "数据清洗", "报表", "连接数据库", "连接 mysql", "连接 pg"]):
            parsed["scenario"] = "data_analysis"
        elif any(k in query_lower for k in ["代码", "编程", "开发", "code", "coding", "programming", "agent开发助手"]):
            parsed["scenario"] = "code_development"
        elif any(k in query_lower for k in ["小说", "故事", "章节续写", "创意写作", "文案", "视频脚本"]):
            parsed["scenario"] = "creative"
